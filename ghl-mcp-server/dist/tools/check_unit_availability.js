...
export async function checkUnitAvailabilityHandler(args) {
    const units = await getUnits();
    const available = units.filter((u) => u.status === 'available');
    if (available.length === 0) {
        return {
            content: [{ type: 'text', text: 'No units are currently available.' }],
        };
    }
    ...
    const lines = filtered.map((u) => `Unit ${u.unitNumber}: ${u.size} (${u.sqFt} sq ft) — $${u.monthlyRate}/month${u.notes ? ` — ${u.notes}` : ''}`);
    return {
        content: [
            {
                type: 'text',

...
export async function getAllPricingHandler(_args) {
    ...
    for (const u of units) {
        ...
        if (!existing) {
            ...
            if (u.monthlyRate < existing.minRate)
                existing.minRate = u.monthlyRate;
        }
    }
    if (sizeMap.size === 0) {
        return {
            content: [{ type: 'text', text: 'No unit pricing information available.' }],
        };
    }
    const lines = Array.from(sizeMap.entries())
        .sort((a, b) => a[1].sqFt - b[1].sqFt)
        .map(([size, info]) => `${size} (${info.sqFt} sq ft): $${info.minRate}/month — ${info.available} of ${info.total} available`);
    return {
        content: [
            {
                type: 'text',

...
function registerSharedGhlTools(server, tenant) {
    // ── Contact Management ──────────────────────────────────────────────────
    server.tool('search_contact', 'Search GoHighLevel CRM contacts by name, email, or phone number.', { query: z.string().describe('The name, email address, or phone number to search for') }, makeSearchContactHandler(tenant));
    server.tool('get_contact', 'Get full contact details by ID, including custom fields (webhook variables) and tags.', { contact_id: z.string().describe('The GHL Contact ID') }, makeGetContactHandler(tenant));
    server.tool('create_contact', 'Create a new contact in GoHighLevel CRM.', {
    ...
    server.tool('update_contact', 'Update a contact\'s fields in GoHighLevel, including custom fields (webhook variables).', {
    ...
    server.tool('delete_contact', 'Delete a contact from GoHighLevel CRM. Use for cleaning up test contacts.', { contact_id: z.string().describe('The GHL Contact ID to delete') }, makeDeleteContactHandler(tenant));
    // ── Notes ───────────────────────────────────────────────────────────────
    server.tool('add_note', 'Add a note to a contact\'s record in GoHighLevel.', {
        contact_id: z.string().describe('The GHL Contact ID'),
        note: z.string().describe('The note text to add'),
    }, makeAddNoteHandler(tenant));
    server.tool('get_contact_notes', 'Get all notes on a contact\'s record. Use to verify call summaries were logged.', { contact_id: z.string().describe('The GHL Contact ID') }, makeGetContactNotesHandler(tenant));
    // ── Tags ────────────────────────────────────────────────────────────────
    server.tool('add_tag', 'Add one or more tags to a contact in GoHighLevel.', {
        contact_id: z.string().describe('The GHL Contact ID'),
        tags: z.string().describe('Comma-separated list of tags to add'),
    }, makeAddTagHandler(tenant));
    // ── Messaging ───────────────────────────────────────────────────────────
    server.tool('send_sms', 'Send an SMS message to a contact in GoHighLevel.', {
        contact_id: z.string().describe('The GHL Contact ID'),
    ...
    server.tool('send_email', 'Send an email to a contact in GoHighLevel (e.g. lien notifications, records confirmations).', {
        contact_id: z.string().describe('The GHL Contact ID'),
    ...
    server.tool('get_messages', 'Get SMS/email message history for a contact. Use to verify messages were sent.', {
        contact_id: z.string().describe('The GHL Contact ID'),
        limit: z.number().optional().describe('Max messages to return (default 20)'),
    }, makeGetMessagesHandler(tenant));
    // ── Workflows ───────────────────────────────────────────────────────────
    server.tool('list_workflows', 'List all workflows in the GHL location. Use to discover workflow IDs.', {}, makeListWorkflowsHandler(tenant));
    server.tool('trigger_workflow', 'Trigger a GoHighLevel workflow for a contact.', {
        contact_id: z.string().describe('The GHL Contact ID'),
        workflow_id: z.string().describe('The GHL Workflow ID to trigger'),
    }, makeTriggerWorkflowHandler(tenant));
    // ── Custom Fields ───────────────────────────────────────────────────────
    server.tool('get_custom_fields', 'List all custom field definitions for the location. Use to discover webhook variable field keys and IDs.', {}, makeGetCustomFieldsHandler(tenant));
}
// ── AIOS-only tool definitions (Google Sheets inventory) ────────────────────
function registerAiosTools(server) {
    server.tool('check_unit_availability', 'Check which self storage units are currently available. Optionally filter by unit size.', { size: z.string().optional().describe('Unit size to filter by, e.g. "10x10"') }, checkUnitAvailabilityHandler);
    server.tool('get_all_pricing', 'Get a full list of all storage unit sizes and their monthly rental rates.', {}, getAllPricingHandler);
    server.tool('get_tenant_info', 'Look up an existing tenant\'s unit information by name, unit number, or GHL contact ID.', {
    ...
    server.tool('update_unit', 'Update a storage unit record in Google Sheets (status, tenant, notes).', {
...
// ── MCP request handler ────────────────────────────────────────────────────
async function handleMcpRequest(tenant, serverName, extraTools, req, res) {

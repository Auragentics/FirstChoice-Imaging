...
- **Medical Records:**
    ...
    - **Step 5: Complete Workflow:** Set `{{contact.intents_handled}}` = `"medical_records"`. If you already completed a previous workflow (e.g., billing), set `{{contact.intents_handled}}` to include both, separated by a comma (e.g., `"billing,medical_records"`).
- **Billing:**
...
- **Lien Request:**
    ...
    - **Step 7: Complete Workflow:** Set `{{contact.intents_handled}}` = `"lien"`. If you already completed a previous workflow (e.g., medical_records), set `{{contact.intents_handled}}` to include both, separated by a comma (e.g., `"medical_records,lien"`).
...
**Agent:** [Action: Send SMS with records request web form link. Stores: `{{contact.records_sms_link_sent}}` = "YES", sets `{{contact.intents_handled}}` = "medical_records"]

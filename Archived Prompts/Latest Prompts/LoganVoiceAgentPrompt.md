...
- **Medical Records:**
    ...
    - **Step 4:** Add "records" to `{{contact.intents_handled}}`.
...
Caller needs records AND has a billing question → Complete records workflow (privacy screening → patient info → web form → intents_handled = "records") → "Anything else?" → Billing workflow (patient info → reason → intents_handled = "records,billing") → "Anything else?" → No → Pre-close check → Confirmation → SMS consent → Close.

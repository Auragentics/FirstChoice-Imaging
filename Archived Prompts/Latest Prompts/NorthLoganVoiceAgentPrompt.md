...
- **Medical Records:**
    ...
    - **Step 4: Complete Workflow:** Set `{{contact.intents_handled}}` = `"records"`. If you already completed a previous workflow (e.g., billing), set `{{contact.intents_handled}}` to include both, separated by a comma (e.g., `"billing,records"`).
- **Billing:**
...
- **Lien Request (Attorney Direct Lien):**
    ...
    - **Step 7: Complete Workflow:** Set `{{contact.intents_handled}}` to include `"lien"`. If you already completed a previous workflow, append it separated by a comma (e.g., `"records,lien"`).
...
**Agent:** "I've sent a link to the medical records request form to your phone." [Trigger SMS tool action] [Stores: `{{contact.records_sms_link_sent}}` = "YES", sets `{{contact.intents_handled}}` = "records"]
